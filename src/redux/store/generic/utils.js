...
export const insertItem = ({ state, payload: { index, item } }) => [
  ...state.slice(0, index),
  item,
  ...state.slice(index),
];

export const removeItem = ({ state, payload: { index } }) => [
  ...state.slice(0, index),
  ...state.slice(index + 1),
];
...
export const updateItem = ({ state, payload: { index, item } }) =>
  state.map((curItem, curIndex) => {
    if (curIndex !== index) return curItem;

    return { ...curItem, ...item };
  });

export const updateItemByKey = ({ state, payload: { item }, key }) =>
  state.map((curItem) => {
...
export const updateItemsByKey = ({ state, payload: { items }, key }) =>
  state.map((curItem) => {
...
export const updateValueAllItems = ({ state, payload }) =>
  state.map((curItem) => ({ ...curItem, ...payload }));
